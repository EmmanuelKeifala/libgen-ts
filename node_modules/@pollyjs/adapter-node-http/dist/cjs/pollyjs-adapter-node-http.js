/**
* @pollyjs/adapter-node-http v6.0.6
*
 ...
 */
const expand = input =>
  Object.entries(input).reduce((acc, [k, v]) => lodash_set(acc, k, v), {});
...
class HttpAdapter extends Adapter {
  ...
  setupNock() {
    ...
    utils.HTTP_METHODS.forEach(m => {
      ...
      interceptor.intercept(/.*/, m).reply(function (_, _body, respond) {
        ...
        const parsedArguments = common_18(...REQUEST_ARGUMENTS.get(req));
        const url = getUrlFromOptions(parsedArguments.options);
        const requestBodyBuffer = Buffer.concat(req.requestBodyBuffers);
        const body = common_15(requestBodyBuffer) ? requestBodyBuffer.toString('utf8') : requestBodyBuffer;
  ...
  patchOverriddenMethods() {
    ...
    keys$1(modules).forEach(moduleName => {
      ...
      function parseArgs() {
        ...
        if (moduleName === 'https') {
          args.options = _objectSpread(_objectSpread({}, {
            port: 443,
            protocol: 'https:',
  ...
  async onFetchResponse(pollyRequest) {
    ...
    const request = new this.NativeClientRequest(_objectSpread(_objectSpread({}, options), {}, {
      method,
  ...
  getBodyFromChunks(chunks, headers) {
    ...
    if (common_10(headers)) {
      ...
      return {
        encoding: 'base64',
        body: stringify$1(encodedChunks)
      };
    }

    const buffer = mergeChunks(chunks);
    const isBinaryBuffer = !common_15(buffer); // The merged buffer can be one of two things:
