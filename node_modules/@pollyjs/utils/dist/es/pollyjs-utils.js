...
function parseQuery(query, options) {
  return qs.parse(query, _objectSpread({
    plainObjects: true,
...
function stringifyQuery(obj, options = {}) {
  return qs.stringify(obj, _objectSpread({
    addQueryPrefix: true,
    strictNullHandling: true
  }, options));
}
...
export { actions as ACTIONS, expiryStrategies as EXPIRY_STRATEGIES, httpMethods as HTTP_METHODS, httpStatusCodes as HTTP_STATUS_CODES, modes as MODES, PollyError, index as Serializers, URL, assert, buildUrl, cloneArrayBuffer, isBufferUtf8Representable, timeout, timestamp };
